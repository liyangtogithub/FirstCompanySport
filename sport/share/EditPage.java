		...
		TextWatcher, Callback
{
	...
	// 文本编辑框
	private TextView etContent1;
	private TextView etContent2;
	...
	{		
		// 标题栏
				llTitle = (TitleLayout) activity.findViewById(R.id.share_title);
				llTitle.setBackgroundResource(R.drawable.title_back);
				llTitle.getBtnBack().setVisibility(View.VISIBLE);
				llTitle.getBtnBack().setOnClickListener(this);
				llTitle.getTvTitle().setText(R.string.share_title_str);
		...
		// 页面主体
		LinearLayout llBody = (LinearLayout) activity.findViewById(R.id.share_body);

		// 文字输入区域
		etContent1 = (TextView) activity.findViewById(R.id.share_content1);
		etContent1.setText(String.valueOf(reqData.get("title")));

		// 说的内容
		etContent = (EditText) activity.findViewById(R.id.share_content2);
		
		// 分享到
		LinearLayout llContent1 = (LinearLayout) activity.findViewById(R.id.share_to_linear);
		...
		HorizontalScrollView sv = (HorizontalScrollView) activity.findViewById(R.id.share_to_img); 
		sv.setHorizontalScrollBarEnabled(false);
		sv.setHorizontalFadingEdgeEnabled(false);

		llPlat = (LinearLayout) activity.findViewById(R.id.share_to_view); 
		/*llPlat.setLayoutParams(new HorizontalScrollView.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.FILL_PARENT));
		sv.addView(llPlat);*/
	
		//Button shareButton = (Button) activity.findViewById(R.id.share_to_button);
	...
	public void onClick(View v)
	{
		...
		{
			String text = etContent1.getText().toString()
			...
			{
				showToast.normaltoast(getContext(), getContext().getString(R.string.select_one_plat_at_least), showToast.ONE_SECOND);
			}
			return;
		}
	...
	public void afterPlatformListGot()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			LinearLayout.LayoutParams lpShareTo = new LinearLayout.LayoutParams(
					20, 20);
			tView.setLayoutParams(lpShareTo);
			llPlat.addView(tView);
	...
	{
		int remain = MAX_TEXT_COUNT - etContent.length() - etContent1.length();
